Ext.define( 'extspect.view.tree.datalist.TreeList',
	{  extend : 'extspect.view.ExtSpectDataList',
		xtype : 'treelist',

		requires : ['extspect.store.tree.ListingStore' , 'extspect.store.tree.TreeListStore'],
		...
		createComponentRowObject : function ( component ) { return { text : this.computeRowObjectString( component ), value : component }; },
		...
		computeRowObjectString : function ( object ) {
			...
			var string = StringOf.to$( object );
			counts[ counts.length - 1 ]++;
			return this.fetchParentNavigationView().showListing ?
				string :
				(   ExtSpect.getUseTreeWithLines() ?
					this.computeVerticalBars().join( '' ) + this.computeObjectString( object ) :
					Ext.String.repeat( ExtSpect.getTreeIndentingChar(), depth ) + string
					);
		},

		computeObjectString : function ( object ) { return '<span style="font-weight:bold">' + StringOf.to$( object ) + '</span>'; },
